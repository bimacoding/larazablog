...
@section('new-layout')
    ...
    <div class="alzatheme-cart-section">
        <div class="container">
            <div class="alzatheme-cart-list">
                <table>
                    <thead>
                    <tr>
                    ...
                    @foreach($carts as $cart)
                        ...
                        <tr data-aos="fade-up" data-aos-duration="400">
                            <td>
                                <div class="alzatheme-cart-thumb">
                            ...
                            <td>
                                <div class="alzatheme-product-number">
                                    <span class="alzatheme-product-minus" data-id="{{ $cart->id }}"><i class="ri-subtract-line"></i></span>
                                    <input type="text" class="number" id="quantity-{{ $cart->id }}" value="{{ $cart->quantity }}"/>
                                    <span class="alzatheme-product-plus" data-id="{{ $cart->id }}"><i class="ri-add-line"></i></span>
                                </div>
                            </td>
    ...
    <div class="section alzatheme-section-padding-bottom">
        <div class="container">
            <div class="alzatheme-cart-total">
                <h5>{{ __('translate.Cart Totals') }}</h5>
                <div class="alzatheme-cart-total-item sub_total">
                    <p>{{ __('translate.Subtotal') }}:</p>
                    <p>{{ currency($subtotal) }}</p>
                </div>

                <div class="alzatheme-cart-total-item total_sale">
                    <p>{{ __('translate.Total') }}:</p>
                    <p><span>{{ currency($subtotal) }}</span></p>
                </div>

                @if($carts->isNotEmpty())
                <a class="alzatheme-default-btn rt-mt-40" data-aos="fade-up" data-aos-duration="800" href="{{ route('checkout.index') }}"
...
@push('js_section')
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            document.querySelectorAll(".alzatheme-product-minus, .alzatheme-product-plus").forEach(span => {
                span.addEventListener("click", function() {
                    ...
                    if (this.classList.contains("alzatheme-product-minus")) {
                    ...
                    } else if (this.classList.contains("alzatheme-product-plus")) {
                        currentQuantity++;
                    }
