...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $partners = Partners::latest()->take(6)->get();

        $showrooms = ShowRoom::latest()->take(6)->get();
        ...
        $theme_view_mapping = [
            ...
            'landing_page' => 'frontend.templates.landing_page',
        ];

        // Default to main_demo if theme is not found
        $view_template = $theme_view_mapping[$selected_theme] ?? 'frontend.templates.landing_page';
    ...
    public function showrooms() {
        $showrooms = ShowRoom::where('active', 1)->latest()->paginate(6); // or any limit
        return view('showroom', compact('showrooms'));
    }



    public function showRoomsDetil($id)
    {

        $shwRoom = ShowRoom::where('id', $id)->firstOrFail();
        $pageTitle = "Show Room";

        return view('showroom_detil', compact('shwRoom', 'pageTitle'));
    }
